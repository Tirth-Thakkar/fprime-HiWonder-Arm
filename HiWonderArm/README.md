# HiWonderArm Application

This deployment was auto-generated by the F' utility tool.

## Building and Running the HiWonderArm Application

In order to build the HiWonderArm application, or any other F´ application, we first need to generate a build directory. This can be done with the following commands:
...
The next step is to build the HiWonderArm application's code.
```
fprime-util build
```
...
To run the ground system without starting the HiWonderArm app:
```
...
```
cd HiWonderArm/build-artifacts/<platform>/bin/
./HiWonderArm -a 127.0.0.1 -p 50000
```
